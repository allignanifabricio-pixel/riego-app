...
function renderSemana() {
  ...
  lunes.setDate(hoy.getDate() - ((hoy.getDay() || 7) - 1) + semanaOffset * 7);

  tituloSemana.textContent =
    `Semana del ${lunes.toISOString().split("T")[0]}`;
  ...
  for (let i = 0; i < 7; i++) {
    ...
    dia.className = "dia";
    dia.innerHTML = `<b>${fecha.toLocaleDateString("es-ES", { weekday: "long" })}</b> – ${iso}`;
...
function editar(id) {
  const r = (JSON.parse(localStorage.getItem("riegos")) || []).find(r => r.id === id);
  if (!r) return;
