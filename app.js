...
document.getElementById("modoOscuro").onclick = () => {
  ...
  localStorage.setItem("oscuro", document.body.classList.contains("oscuro"));
};
...
/* 📆 render semana (SIEMPRE muestra los 7 días) */
function renderSemana() {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    // riegos de ese día
    const riegosDelDia = riegos.filter(r => tocaRiego(r, iso));

    if (riegosDelDia.length === 0) {
      const sin = document.createElement("div");
      sin.className = "sin-riego";
      sin.textContent = "Sin riego";
      dia.appendChild(sin);
    } else {
      riegosDelDia.forEach(r => {
...
/* ✏️ editar árbol */
function editar(id) {
  const r = (JSON.parse(localStorage.getItem("riegos")) || []).find(
    r => r.id === id
  );
  if (!r) return;

  fechaInicio.value = r.inicio;
  arbol.value = r.arbol;
  frecuencia.value = r.frecuencia;
